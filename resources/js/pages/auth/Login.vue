...
<template>
    <AuthBase :title="__('auth.login.title')" :description="__('auth.login.description')">
        <Head :title="__('auth.login.head')" />
        ...
        <form @submit.prevent="submit" class="flex flex-col gap-6">
            ...
            <div v-if="registrationEnabled" class="text-center text-sm text-muted-foreground">
                ...
                <TextLink :href="route('register')" :tabindex="5">{{ __('auth.login.signup') }}</TextLink>
            </div>
        </form>
    </AuthBase>
</template>
